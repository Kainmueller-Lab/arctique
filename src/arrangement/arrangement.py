...
class CellArrangement:
    """
    Base class for cell arrangements. A cell arrangement is a collection of cells placed in a specific way in a volume mesh.
    """
    ...
    def __init__(self):
        """
        Initializes a CellArrangement object.
        """
...
class VolumeFill(CellArrangement):
    ...
    Parameters:
        - mesh (Mesh): mesh of volume to populate with nuclei
        - density (float): density of nuclei to place
        - types (list): list of nuclei type types that should appear
        - ratios (list): list of ratios of nuclei types to populate
        - seed (int): seed for random generation
        - rescaling (float): factor to rescale nuclei sizes
    """
...
class VoronoiFill(CellArrangement):
    ...
    Parameters:
        ...
        - type: nuclei type that should populate the mesh
        - bounding_box: optional bounding box to delete compartments before placing nuclei
        - rescaling: optional rescaling factor for the nuclei size
    """
    ...
    def add_nuclei(self):
        """
        Fills the volume of a mesh with cells of given type.

        Parameters:
            - mesh_obj: object of volume to populate with nuclei
            - surface_obj: surface object of the volume
            - type: nuclei type that should populate the mesh
            - bounding_box: optional bounding box to delete compartments before placing nuclei
            - rescaling: optional rescaling factor for the nuclei size

        Returns:
            - list of created nuclei objects
        """
    ...
    def sort_data_by_root_dist(self, data, v):
        """
        Sorts a list of data points by their distance to a given reference point.

        Parameters:
            data (list of tuples): A list where each tuple contains a data point and additional information.
            v (numpy.ndarray): The reference point to calculate distances from.

        Returns:
            list of tuples: The input data sorted by the distance of each data point to the reference point.
        """
    ...
    def sample_centers(self, verts, dist, max_count):
        ...
        Parameters:
            verts (list): A list of vertex positions.
        ...
        Returns:
            tuple: A tuple containing:
                - sampled_verts (list): The list of sampled vertex positions.
                - sampled_ids (list): The list of indices corresponding to the sampled vertices in the original list.
        """
        sampled_verts = []
        sampled_ids = []
    ...
    # NOTE: This method is necessary for the Arrangement class. - ck
    def add(self):
        pass